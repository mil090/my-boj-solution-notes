...
# 문제
'''
1번부터 N번까지 N개의 풍선이 원형으로 놓여 있고. i번 풍선의 오른쪽에는 i+1번 풍선이 
있고, 왼쪽에는 i-1번 풍선이 있다. 단, 1번 풍선의 왼쪽에 N번 풍선이 있고, N번 풍선의 
오른쪽에 1번 풍선이 있다. 각 풍선 안에는 종이가 하나 들어있고, 종이에는 -N보다 크거나 
같고, N보다 작거나 같은 정수가 하나 적혀있다. 이 풍선들을 다음과 같은 규칙으로 터뜨린다.
우선, 제일 처음에는 1번 풍선을 터뜨린다. 다음에는 풍선 안에 있는 종이를 꺼내어 그 종이에 
적혀있는 값만큼 이동하여 다음 풍선을 터뜨린다. 양수가 적혀 있을 경우에는 오른쪽으로, 
음수가 적혀 있을 때는 왼쪽으로 이동한다. 이동할 때에는 이미 터진 풍선은 빼고 이동한다.
예를 들어 다섯 개의 풍선 안에 차례로 3, 2, 1, -3, -1이 적혀 있었다고 하자. 이 경우 
3이 적혀 있는 1번 풍선, -3이 적혀 있는 4번 풍선, -1이 적혀 있는 5번 풍선, 1이 적혀 
있는 3번 풍선, 2가 적혀 있는 2번 풍선의 순서대로 터지게 된다.
'''
# 입력
'''
첫째 줄에 자연수 N(1 ≤ N ≤ 1,000)이 주어진다. 다음 줄에는 차례로 각 풍선 안의 
종이에 적혀 있는 수가 주어진다. 종이에 0은 적혀있지 않다.
'''
# 출력
'''
...
# 해법
'''
1. N을 입력받음. 결과를 저장할 빈 덱 result를 생성
2. 각 풍선에 들어 있는 종이에 적힌 숫자 N개를 입력받아 덱 numbers로 저장하고, 1부터
N까지의 자연수가 들어 있는 덱(풍선 번호 모음) B를 생성
3. 각 풍선의 번호를 key, 각 풍선에 들어 있는 숫자를 value로 하는 딕셔너리 balloons
를 생성(zip 함수를 이용)
4. 현재 풍선의 위치를 나타낼 변수 idx를 생성하여 0으로 초기화
5. 무한 루프를 생성하여 아래 문장을 반복 실행
B[idx]의 값을 cur로 저장하고, B에서 cur을 제거
balloons에서 key 값이 cur인 쌍을 제거하고, 그 value를 r로 저장(pop 함수를 이용)
result에 str(r)을 삽입
만약 위 문장들을 실행한 후 B가 공백 상태가 되었다면 반복문을 탈출
만약 아직 B가 공백 상태가 아니라면 idx의 이동이 필요함
만약 r이 양수인 경우, idx는 (idx-1+r)%len(B)로 대체
만약 r이 음수인 경우, idx는 (idx+r)%len(B)로 대체
6. result의 모든 원소들을 공백 간격으로 출력
'''
from collections import deque
N=int(input())
...
B=deque([x for x in range(1, N+1)])
balloons=dict(zip(B, numbers))
idx=0
while True:
    cur=B[idx]
    B.remove(cur)
